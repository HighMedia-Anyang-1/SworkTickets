...
// 		        	   element.innerHTML = "<div style='color:red'>a</div>";
		        	   ...
		        	   if(error!=1){
		        		$.each(UserExhibitionList, function(i, v){
							...
							var st_Date = new Date(exh_st_date);
							var end_Date = new Date(exh_end_date );
							var st_re_date= st_Date.getFullYear()+"-"+(st_Date.getMonth()+1)+"-"+("00"+st_Date.getDate().toString()).slice(-2);
							var en_re_date= end_Date.getFullYear()+"-"+(end_Date.getMonth()+1)+"-"+end_Date.getDate();
							var rank_cnt= UserExhibitionList[i].rank_cnt;		
							
							comment_html +="<div class='col' style='margin-bottom: 30px; cursor:pointer;' >";
							comment_html +="<a href=getUserExhibition?exh_no="+exh_no+" style='text-decoration: none;'>";
...
// 				            comment_html  +=    exh_st_date+"~"+exh_end_date;
				            comment_html  +=    st_re_date+"~"+en_re_date;
...
<body class="d-flex flex-column min-vh-100">
   ...
   <c:forEach items="${loc}" var="loc_name" varStatus="status">
            <li class="loc_name" id="${status.count}" onclick="local_func('${loc_name}')"><a href="#tabs-1" id="loc_name" >${loc_name}</a></li> 
	</c:forEach>
         </ul>
         <div  id="tabs-1" class="container">
		<div id="tab-r" class="row row-cols-1 row-cols-md-4 g-4" style="margin-bottom: 30px;">
